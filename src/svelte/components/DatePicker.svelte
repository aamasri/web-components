<script>
    ...
    export let min = new Date(defaultDate.getFullYear() - 3, 0, 1);
    /** The latest year the user can select */
    export let max = new Date(defaultDate.getFullYear() + 3, 11, 31, 23, 59, 59, 999);
    $: if (value && value > max) {
        setValue(max);
    ...
    <div class="tab-container">
        <div class="top">
            ...
            <div class="dropdown month">
                <select bind:value={browseMonth} on:change={() => { setMonth(browseMonth) }} on:keydown={monthKeydown}>
                    {#each iLocale.months as monthName, i}
                        <option disabled={ new Date(browseYear, i, getMonthLength(browseYear, i), 23, 59, 59, 999) < min || new Date(browseYear, i) > max }
...
<style>
    ...
    .date-time-picker:focus {
        ...
        box-shadow: 0 0 0 2px var(--date-picker-highlight-shadow, rgba(2, 105, 247, 0.4));
    }
